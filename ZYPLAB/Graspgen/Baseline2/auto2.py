...
# 隔离的 Conda 环境和 Baseline2 后处理脚本路径
...
simulation_app = SimulationApp({"headless": True}) # Baseline2 原设为 True
...
# ===================== 3. 核心实验大循环 =====================
print(f"🚀 [3/3] 开始执行 Baseline2 自动化抓取评测，总计 {TOTAL_TRIALS} 轮...")

for trial in range(TOTAL_TRIALS):
    cam_id = (trial % 7) + 1 
    start_time = time.time()
    print(f"\n" + "="*60)
    print(f"🔄 正在启动第 {trial}/{TOTAL_TRIALS-1} 轮... (cam{cam_id}.usd | 任务: {task_name} | 物体: {obj_class})")
    print(f"="*60)
    ...
    # 🌟 优化核心 2：推理结束，立马搬回 CPU 内存，并彻底清空 GPU 显存
    del inference_state
    del output_obj
    sam3_model.to('cpu')
    ...
    save_cam_img(camera, os.path.join(IMG_DIR, f"trial_{trial:03d}_cam{cam_id}_{task_name}_step0_before_grasp.png"))

    print(">>> 步骤 1: 插入并闭合夹爪...")
    # Baseline2 插入深度 0.115m
    move_to_pose(ik_solver, franka, world, grasp_pos + grasp_dir * 0.115, grasp_quat, steps=80)
    ...
    print(">>> 步骤 2: 提起物体...")
    # Baseline2 提升高度 0.15m
    move_to_pose(ik_solver, franka, world, grasp_pos + np.array([0, 0, 0.15]), grasp_quat, steps=120)
