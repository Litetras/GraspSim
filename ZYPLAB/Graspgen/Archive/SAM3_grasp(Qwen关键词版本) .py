...
print("\n🧠 正在呼叫本地 Qwen 大模型计算抓取方向...")
llm_prompt = "grasp the knife to cut the bread,tell me the Direction of Palm,just answer up or down"

# 使用我们调优过的无思考、零温度度的 API 调用
response = ollama.chat(
    model='qwen3.5:4b',
    messages=[
        {
            'role': 'system', 
            'content': 'You are a strict robotic parser. You MUST NOT output <think> tags. You MUST NOT reason or explain. Output ONLY one word.'
        },
...
# 物理切除 <think> 标签，去掉首尾空格、换行符和句号，并强制转为小写
clean_direction = re.sub(r'<think>.*?</think>', '', raw_output, flags=re.DOTALL).strip(" .\n").lower()

print(f"✅ 大模型决策完成！动态分配抓取指令: '{clean_direction}'\n")

# 将大模型的输出直接传给你的网络
target_instruction = clean_direction
###############################################################################################################
grasp = demo_variable(
    rgb_data=rgb_data, 
