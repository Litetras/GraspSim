...
# ===================== 3. 核心实验大循环 =====================
print(f"🚀 [3/3] 开始执行 Baseline4 (ShapeGrasp) 自动化抓取评测，总计 {TOTAL_TRIALS} 轮...")
...
for trial in range(TOTAL_TRIALS):
    ...
    # ---------------- 3.3 交互临时文件存储 ----------------
    TEMP_IN  = f"/tmp/sg_in_trial{trial}.npz"
    TEMP_OUT = f"/tmp/sg_out_trial{trial}.npz"

    if os.path.exists(TEMP_OUT): os.remove(TEMP_OUT)
    np.savez(TEMP_IN, depth=depth_data, K=cam_K, segmap=final_mask, rgb=rgb_data)

    # ---------------- 3.4 跨环境调用 ShapeGrasp 子进程 ----------------
    print(">>> 正在调用 ShapeGrasp 大模型推理，请稍候...")
    res_sg = subprocess.run([SG_PYTHON, WORKER_SCRIPT, "--in_data", TEMP_IN, "--out_data", TEMP_OUT], 
                            env=my_env, cwd="/home/zyp/pan1/ShapeGrasp")
    
    if res_sg.returncode != 0 or not os.path.exists(TEMP_OUT):
        print("❌ ShapeGrasp 运行失败，跳过本轮！")
        continue

    res_data = np.load(TEMP_OUT, allow_pickle=True)
    if not res_data['success']:
        print("❌ ShapeGrasp 未能生成有效抓取，跳过本轮！")
        continue

    # 提取 ShapeGrasp 结果：[Angle, U, V, Depth]
    sg_result = res_data['grasp_2d']
    angle_deg, u, v, z_depth = sg_result[0], sg_result[1], sg_result[2], sg_result[3]
    ...
    T_world_grasp = T_world_cam @ to_T44(FLIP_CAM) @ T_cam_grasp @ to_T44(R_user_tune)     
    ...
    print("🔍 坐标轴 Marker 已生成")

    # 执行动作
    hover_pos = grasp_pos - grasp_dir * 0.12 # ShapeGrasp 的悬停补偿
    print(f">>> 步骤 0: 移动到悬停点 {np.round(hover_pos, 3)}...")
    if not move_to_pose(ik_solver, franka, world, hover_pos, grasp_quat, steps=150, label=" [悬停]"):
        continue
    
    insert_pos = grasp_pos + grasp_dir * 0.02 # ShapeGrasp 的进近补偿
    ...
    if not move_to_pose(ik_solver, franka, world, insert_pos, grasp_quat, steps=100, label=" [进近]"):
        continue
