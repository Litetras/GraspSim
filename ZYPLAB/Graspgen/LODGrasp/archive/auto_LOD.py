...
for cam_id in range(1, 8):
    usd_path = os.path.join(SCENE_DIR, f"cam{cam_id}_r.usd")
    print("\n" + "=" * 60)
    ...
    for trial in range(1, 15):
        print("\n" + "-" * 50)
        print(f"🎥 场景: cam{cam_id} | 🔄 测试轮次: {trial} / 14")
        print("-" * 50)
        ...
        # ================== SAM3 图像处理 ==================
        print(f"[{time.strftime('%H:%M:%S')}] 加载 SAM3 模型...")
        ...
        if len(masks) == 0:
            ...
            del sam3_model, sam3_processor, inference_state_obj; gc.collect(); torch.cuda.empty_cache()
            continue
        ...
        # 卸磨杀驴，确保显存安全
        print("🧹 销毁 SAM3 释放显存...")
        del sam3_model, sam3_processor, inference_state_obj
        gc.collect()
        ...
        # 步骤 2
        print(">>> 步骤 2: 提起物体...")
        move_to_pose(grasp_pos + np.array([0, 0, 0.15]), grasp_quat, step_count=120)
