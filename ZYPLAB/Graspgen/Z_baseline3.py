...
# ===================== 交互式指令输入 =====================
# 提前在最前面输入，避免被物理引擎的日志刷屏淹没
print("\n" + "="*50)
#task_ins_txt = input('🤖 请输入自然语言指令 (e.g., grasp the knife to cut): ').lower()
task_ins_txt = "grasp the knife to cut".lower()
...
if "cut" in task_ins_txt and "knife" in task_ins_txt:
    ...
    task_name, obj_class = "hammer", "hammer"
print(f"✅ 解析结果 -> 任务: {task_name}, 物体: {obj_class}")
print("="*50 + "\n")
...
WORKER_SCRIPT = "/home/zyp/IsaacLab/ZYPLAB/Graspgen/Z_cgn_worker_baseline3.py" 

GRASPGPT_PYTHON = "/home/zyp/anaconda3/envs/graspgpt/bin/python"
GPT_WORKER_SCRIPT = "/home/zyp/IsaacLab/ZYPLAB/Graspgen/Z_graspgpt_worker_baseline3.py"

TEMP_IN = "/tmp/cgn_in.npz"       # 输入给 CGN 的数据
TEMP_CGN_OUT = "/tmp/cgn_out.npz" # CGN 输出（也是 GPT 的输入）
TEMP_GPT_OUT = "/tmp/gpt_out.npz" # GPT 最终筛选出的最优抓取
...
res_data = np.load(TEMP_CGN_OUT, allow_pickle=True) # 这里改为 TEMP_CGN_OUT
...
# ===================== 跨环境调用 GraspGPT =====================
print(f">>> [主程序] 跨进程启动 GraspGPT (根据自然语言 '{task_name}' 筛选最优抓取)...")
result_gpt = subprocess.run(
    [GRASPGPT_PYTHON, GPT_WORKER_SCRIPT, 
     ...
     "--task", task_name,
     "--obj_class", obj_class],
    env=my_env  # 继续使用干净的环境变量，防止带入主程序的包路径
)
...
print(">>> 步骤: 插入并抓取...")
move_to_pose(grasp_pos + grasp_dir * 0.125, grasp_quat, steps=80)# 增加插入深度到 12cm
