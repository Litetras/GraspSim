...
# ===================== 新增：方向筛选辅助函数 =====================
def filter_grasps_by_orientation(grasps, scores, target_vector, angle_threshold_deg=30):
    """
    根据目标向量筛选抓取
    :param grasps: (N, 4, 4) 抓取矩阵
    :param scores: (N,) 抓取分数
    :param target_vector: (3,) 目标方向向量
    :param angle_threshold_deg: 允许的最大偏差角度（度）
    :return: 筛选后的 grasps, scores
    """
    # 1. 归一化目标向量
    ...
    # 2. 提取所有抓取的接近方向 (Z轴: 旋转矩阵的第三列)
    # grasps shape: [N, 4, 4], 取 [:3, 2] -> [N, 3]
    approach_vectors = grasps[:, :3, 2]

    # 3. 计算点积 (余弦相似度)
    # dot_products shape: [N]
    dot_products = np.sum(approach_vectors * target_vec, axis=1)

    # 4. 计算阈值 (cos(angle))
    # 注意：这里假设必须同向（方向一致），如果允许反向（如抓棍子），可取 abs(dot_products)
    threshold_val = np.cos(np.deg2rad(angle_threshold_deg))

    # 5. 筛选
    valid_mask = dot_products > threshold_val
    
    filtered_grasps = grasps[valid_mask]
    filtered_scores = scores[valid_mask]
    
    print(f"Orientation Filter: {len(grasps)} -> {len(filtered_grasps)} remaining (Threshold: {angle_threshold_deg}°, Target: {target_vec})")
...
def demo_variable(
    ...
    # --- 【新增】方向引导参数 ---
    target_vector: Optional[List[float]] = None,  # 目标方向向量
    angle_threshold: float = 30.0,                # 允许的角度偏差

    # --- 配置参数 ---
    gripper_config: str = "/home/zyp/GraspGen/models/checkpoints/graspgen_franka_panda.yml",
    grasp_threshold: float = 0.6,  # 0.8,##############################
    num_grasps: int = 200,
    return_topk: bool = False,
    topk_num_grasps: int = -1,
    collision_threshold: float = 0.0001,
    ...
    """
    核心接口：支持 'RGBD+Mask' 或 '直接点云' 两种输入模式，并支持方向筛选。
    """
    ...
    grasps_inferred[:, 3, 3] = 1 # 确保齐次矩阵合法
    print(f"Inferred {len(grasps_inferred)} raw grasps.")

    # ===================== 【新增】方向筛选逻辑 =====================
    if target_vector is not None:
        print(f"Applying orientation filter...")
        grasps_inferred, grasp_conf_inferred = filter_grasps_by_orientation(
            grasps_inferred, 
            grasp_conf_inferred, 
            target_vector, 
        ...
        if len(grasps_inferred) == 0:
            raise ValueError(f"方向筛选后无剩余抓取！请尝试增大 angle_threshold (当前: {angle_threshold}°)")
    # ==============================================================
    ...
    if len(collision_free_grasps) == 0:
        raise ValueError("无无碰撞抓取！(可能是方向筛选后留下的抓取都发生了碰撞)")
