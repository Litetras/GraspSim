...
# 新增：图片保存配置
SAVE_DIR = "/home/zyp/Desktop"  # 自定义保存目录
os.makedirs(SAVE_DIR, exist_ok=True)  # 确保目录存在
OBJECT_IMAGE_NAME = "segmented_object_transparent.png"  # 透明背景物体图片
HANDLE_IMAGE_NAME = "segmented_handle_transparent.png"  # 透明背景手柄图片
FINAL_MASK_IMAGE_NAME = "final_mask.png"  # 最终mask图片文件名
...
# 方案B：若SAM3无set_point_prompt方法，改用文字提示+选点后处理（兼容方案）
...
# ===================== 新增：保存透明背景的物体图片 =====================
# 创建RGBA图像（RGB + Alpha通道）
# Alpha通道：物体区域255（不透明），背景区域0（完全透明）
object_rgba = np.dstack((object_rgb, object_mask * 255))  # 添加Alpha通道
object_image_pil = Image.fromarray(object_rgba.astype(np.uint8))

# 保存透明背景图片（必须用PNG格式，JPG不支持透明）
object_image_save_path = os.path.join(SAVE_DIR, OBJECT_IMAGE_NAME)
object_image_pil.save(object_image_save_path, format='PNG')
print(f"透明背景的物体图片已保存至：{object_image_save_path}")

# 可选：同时保存物体mask为图片（方便查看）
object_mask_save_path = os.path.join(SAVE_DIR, "object_mask.png")
cv2.imwrite(object_mask_save_path, object_mask * 255)  # 乘以255让mask可见
print(f"物体mask图片已保存至：{object_mask_save_path}")
...
if len(handle_masks) > 0:
    ...
    # 保存透明背景的手柄图片
    handle_rgb = np.where(np.repeat(handle_mask[:, :, np.newaxis], 3, axis=2) == 1, object_rgb, 0)
    # 创建RGBA手柄图像
    handle_rgba = np.dstack((handle_rgb, handle_mask * 255))  # Alpha通道控制透明度
    handle_image_pil = Image.fromarray(handle_rgba.astype(np.uint8))
    
    # 保存透明背景手柄图片
    handle_image_save_path = os.path.join(SAVE_DIR, HANDLE_IMAGE_NAME)
    handle_image_pil.save(handle_image_save_path, format='PNG')
    print(f"透明背景的手柄图片已保存至：{handle_image_save_path}")
else:
...
# 保存最终mask图片
final_mask_save_path = os.path.join(SAVE_DIR, FINAL_MASK_IMAGE_NAME)
cv2.imwrite(final_mask_save_path, final_mask * 255)
print(f"最终mask图片已保存至：{final_mask_save_path}")
