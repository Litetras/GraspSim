...
###########################################################################
# 核心替换：SAM → SAM2 分割模块（修复选点和窗口问题，只输出一个mask）
###########################################################################
import sys
sys.path.append(r'/home/zyp/GraspGen')
# 添加手柄分割模块路径（如果两个文件在同一目录，可省略）
sys.path.append(r'/home/zyp/vlpart/VLPart')  # 添加 vlpart 根目录（关键！）
sys.path.append(r'/home/zyp/vlpart/VLPart/demo')  # 添加 handle.py 所在目录
...
# 导入手柄分割模块
from handle import segment_handle####################################
...
# ===================== 新增：5. 手柄分割（传入物体RGB图像） =====================
print("开始执行手柄分割（输入：物体RGB图像）...")
# 调用手柄分割函数（修改：传入提取的物体RGB图像，输出手柄mask）
handle_mask = segment_handle(rgb_image=object_rgb, confidence_threshold=0.5)

# 可视化手柄分割结果
plt.figure("Handle Segmentation Result", figsize=(15, 7))
plt.subplot(131)
plt.imshow(rgb_data)
plt.title("Original RGB")
plt.subplot(132)
plt.imshow(object_rgb)
plt.title("Extracted Object RGB (Input to VLPart)")
plt.subplot(133)
plt.imshow(handle_mask, cmap='gray')
plt.title("Handle Mask (Output from VLPart)")
...
# 可视化最终mask
plt.figure("Final Mask", figsize=(12, 7))
plt.imshow(final_mask, cmap='gray')
plt.title("Final Mask (Object + Handle)")
